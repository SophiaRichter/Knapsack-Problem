...
public class Simulation {
    ...
    private Table<Integer, Integer, GridCell> bestgrid = HashBasedTable.create();
    ...
    private boolean rerender;				// A boolean representing if the solution should be rerendered

    /**
     * Returns the GridCell at the position (x,y) of the best solution found
     * 
     * @param x : the x-position of the desired GridCell
     * @param y : the y-position of the desired GridCell
     * @return the GridCell object of the best solution at position (x,y)
     */
    public GridCell getBestCell(int x, int y) {
    	return bestgrid.get(x,y);
    }
    ...
    public void setup(LearningAlgorithm algorithm, int width, int height, Multiset sack) {
    	boolean fresh = bestgrid.isEmpty();	// stores if the program just started and there is no best solution available
        cols = width;				// save the width of the game-field (in this case always 1)
        rows = height;				// save the height of the game-field
        ...
        for (int y = 0; y < rows; y++) {
            for (int x = 0; x < cols; x++) {
                final GridCell cell = new GridCell(x, y, CellType.NORMAL,null);
                grid.put(x, y, cell);	// initialize the game-grid with normal cells
                if(fresh) bestgrid.put(x,y,cell);	// if the program just started and there hasn't been a best solution yet initialize the best solution with normal cells
            }
        }
        /* ---------------------------------------------------------------- */
    ...
    public void place() {
        ...
        boolean placementSuccessful = handlePlacement(action);
        if(placementSuccessful)
        {
        	...
        	isOver = sack.isEmpty();									// if the sack is empty the algorithm placed every Brick and the game is won
        ...
        else
        {
        	...
        	newState = new State(Long.MAX_VALUE);	// if the episode ended due to an error the next state is an error state
        }
        rerender = algorithm.feedback(currentState, newState, action, reward, isOver);	// give the calculated reward to the LearningAlgorithm
        if (rerender && placementSuccessful) {
        	bestgrid.clear();
        	bestgrid = HashBasedTable.create(grid);
        }
    }
    	...
    	for(int i=0; i<bitlen; i++) {		//for this algorithm it doesn't matter if we start with the columns
    		for(int j=0; j<bitlen; j++) {	// or the rows, since most of the work is done in the class Brick
    			...
    			{ // if none of the above problems occur the CellType of the type GridCell can be set to BLOCKED and the loop can be continued
    				...
    				getCell(i+xPos,j+yPos).col = b.color;
    			}
    		}
    	}
